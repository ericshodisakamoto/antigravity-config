...
- [The official doc about `editor.find.seedSearchStringFromSelection` setting](https://github.com/whitphx/vscode-emacs-mcx/blob/HEAD/basics#_seed-search-string-from-selection)
...
| `M-SPC`                                          | ✓               | Cycle spacing: first reduces whitespace to one space, then deletes all whitespace, then restores original (cycle-spacing). `alt+space` is ignored by VSCode ([issue](https://github.com/microsoft/vscode/issues/199984)), so this keybinding only works with other Meta key options (Escape, Ctrl+[, or Cmd on macOS), which need to be enabled via `emacs-mcx.useMetaPrefixEscape`, `emacs-mcx.useMetaPrefixCtrlLeftBracket`, or `emacs-mcx.useMetaPrefixMacCmd`. |
...
This is mainly for extension developers who want to make the extensions collaborative with this extension's prefix argument. See [the issue [#1146](https://github.com/whitphx/vscode-emacs-mcx/issues/1146)](https://github.com/whitphx/vscode-emacs-mcx/issues/1146) for the discussion about it.
...
Please see [CONTRIBUTING.md](https://github.com/whitphx/vscode-emacs-mcx/blob/HEAD/CONTRIBUTING.md) about development of this extension.
